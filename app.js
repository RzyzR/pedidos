...
db.collection("Plato").onSnapshot((querySnapshot) => {
    ...
    querySnapshot.forEach((doc) => {
        ...
        <td><button class="btn btn-warning" onclick="editar('${doc.id}','${doc.data().cantidad}','${doc.data().descripcion}')">Editar</button></td>
        </tr>
        `
    });
});
...
function eliminar(id){
    db.collection("Plato").doc(id).delete().then(function() {
...
function editar(id,cantidad,descripcion){
    ...
    boton.onclick = function(){
        var washingtonRef = db.collection("Plato").doc(id);
