...
function guardar(){
    ...
    db.collection("PlatoDragon2").add({
        cantidad: cantidad,
...
db.collection("PlatoDragon2").onSnapshot((querySnapshot) => {
    tabla.innerHTML = '';
...
function eliminar(id){
    db.collection("PlatoDragon2").doc(id).delete().then(function() {
...
function editar(id,cantidad,descripcion){
    ...
    boton.onclick = function(){
        var washingtonRef = db.collection("PlatoDragon2").doc(id);
